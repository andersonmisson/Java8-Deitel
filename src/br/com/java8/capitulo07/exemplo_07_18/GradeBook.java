package br.com.java8.capitulo07.exemplo_07_18;

// Figura 07.18: GradeBook.java
...
public class GradeBook {
	...
	public int getMinimum() {
		...
		for(int[] studentGrades : grades) {
			...
			for (int grade : studentGrades) {
				...
				// Se a nota for menos que lowGrade, atribui a nota a lowGrade
				if(grade < lowGrade) {
					lowGrade = grade;
				}
			}
	...
	public void outputGrades() {
		...
		// cria um título de coluna para cda um dos testes
		for(int test = 0; test < grades[0].length; test++) {
			System.out.printf("Test %d ", test + 1);
		}
		
		System.out.println("Average"); // Tituo da coluna de média do aluno
	...
	public void outputBarChart() {
		...
		// Para cada nota em GradeBook, incremnta a frequência apropriada
		for(int[] studentGrades : grades) {
			for(int grade : studentGrades) {
				++frequency[grade / 10];
			}
